...
def render_visualizacao(municipios_selecionados, ano):
    ...
    for municipio_id in municipios_selecionados:
        ...
        orcamento = query_db(f"""
            ...
            AND exercicio_orcamento = {int(str(ano) + '00')}
        """)
        ...
        fig.update_layout(
            title=f"{nome_mapping.get(municipio_id, 'Município Não Encontrado')} ({ano})",
            yaxis_title="Valor (R$)",
